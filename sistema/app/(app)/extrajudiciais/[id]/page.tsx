...
import { getExtrajudicial, getMinutasAssistidasByEntity } from '@/lib/data'
import {
...
import { RESPONSAVEIS, STATUS_EXTRAJUDICIAL, TIPOS_EXTRAJUDICIAL, TIPO_DOCUMENTO_MINUTA_LABELS } from '@/lib/types'
...
export default async function ExtrajudicialPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [e, minutasVinculadas] = await Promise.all([
    getExtrajudicial(id),
    getMinutasAssistidasByEntity('extrajudicial', id),
  ])
  if (!e) notFound()
    ...
    <div className="p-8 max-w-3xl">
      <div className="mb-6 flex items-center justify-between">
        ...
        <form action={excluirComId}>
          <DeleteButton
            confirmMessage="Excluir esta demanda?"
      ...
      <EntityAdvisorPanel entityType="extrajudicial" entityId={e.id} />
      <KnowledgeRecommendations entityType="extrajudicial" entityId={e.id} />
      ...
      <div className="mt-6 bg-white border border-gray-200 rounded p-5">
        <div className="flex items-center justify-between mb-3">
          ...
          <Link
            href={`/minutas/nova?entityType=extrajudicial&entityId=${e.id}`}
