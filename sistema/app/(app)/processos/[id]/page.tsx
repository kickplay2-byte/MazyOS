...
import { getProcesso, getMinutasAssistidasByEntity } from '@/lib/data'
import { atualizarProcesso, excluirProcesso, adicionarMovimentacao } from '@/lib/actions'
import StatusBadge from '@/components/StatusBadge'
import DeleteButton from '@/components/ui/DeleteButton'
import { RESPONSAVEIS, STATUS_PROCESSO, TIPO_DOCUMENTO_MINUTA_LABELS } from '@/lib/types'
...
export default async function ProcessoPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [p, minutasVinculadas] = await Promise.all([
    getProcesso(id),
    getMinutasAssistidasByEntity('processo', id),
  ])
  if (!p) notFound()
    ...
    <div className="p-8 max-w-3xl">
      <div className="mb-6 flex items-center justify-between">
        ...
        <form action={excluirComId}>
          <DeleteButton
            confirmMessage="Excluir este processo?"
      ...
      <EntityAdvisorPanel entityType="processo" entityId={p.id} />
      <KnowledgeRecommendations entityType="processo" entityId={p.id} />
      ...
      <div className="mt-6 bg-white border border-gray-200 rounded p-5">
        <div className="flex items-center justify-between mb-3">
          ...
          <Link
            href={`/minutas/nova?entityType=processo&entityId=${p.id}`}
