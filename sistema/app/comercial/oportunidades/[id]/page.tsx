...
import { getOportunidade, getCliente, getCorretor, getImobiliaria, getClientes, getCorretores, getImobiliarias, getPropostasByOportunidade } from '@/lib/data'
...
import { RESPONSAVEIS, TIPOS_EXTRAJUDICIAL, STATUS_COMERCIAL } from '@/lib/types'
...
const STATUS_PIPELINE_LABELS = [
  'novo_lead', 'primeiro_contato', 'diagnostico_pendente', 'diagnostico_realizado',
  'proposta_enviada', 'follow_up', 'negociacao', 'contrato_enviado', 'fechado',
]

export default async function OportunidadeDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params
  const op = getOportunidade(id)
  if (!op) notFound()

  const cliente = getCliente(op.clienteId)
  const corretor = op.corretorId ? getCorretor(op.corretorId) : undefined
  const imobiliaria = op.imobiliariaId ? getImobiliaria(op.imobiliariaId) : undefined

  const clientes = getClientes()
  const corretores = getCorretores()
  const imobiliarias = getImobiliarias()
  const propostasVinculadas = getPropostasByOportunidade(id)
