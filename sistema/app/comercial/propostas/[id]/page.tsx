...
import { RESPONSAVEIS, TIPOS_EXTRAJUDICIAL, FORMAS_PAGAMENTO } from '@/lib/types'
...
export default async function PropostaDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const corretor = proposta.corretorId ? getCorretor(proposta.corretorId) : undefined
  const imobiliaria = proposta.imobiliariaId ? getImobiliaria(proposta.imobiliariaId) : undefined
  const oportunidade = proposta.oportunidadeId ? getOportunidade(proposta.oportunidadeId) : undefined
